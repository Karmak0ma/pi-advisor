...
- **Repeats**: an exact repeat of an already-answered question is skipped without a Jev call and the earlier advice is reattached. Repeat reattachment is code-side and free — it also works with `advisorJevFilterEnabled` off; the flag gates only the Jev screening call.
